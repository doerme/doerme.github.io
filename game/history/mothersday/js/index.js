...
//})
/*mother*/
$('#js_p1_bt').on('tap',function(){
	goNext();
});

$('#js_p2_bt').on('tap',function(){
    goNext();
});

$('#js_p4_bt1').on('tap',function(){
    goPrev();
    setTimeout(function(){
     goPrev();
    },600);
...
$('#js_p4_bt2').on('tap',function(){
    goNext();
});

$('#js_sf_bt').on('tap',function(){
    $('.img_share_form').show();
});
...
$(".js_select_main").each(function(index, element) {
...
$('.js_select_main li').on('tap',function(){
...
function goNext(){
	...
	last.col = now.col;
	if(last.row==2){
        $('.loading-line').children('.loading-main').addClass('pt-page-mkloading');
	    setTimeout(function(){
	    	goNext();
	    },2000);
	}
    if(last.row==3){
        $('.js_daughter').attr('src','imgm/cons/con_'+$('.constellation_2 > .cur').data('value')+'.png');
        $('.js_mother').attr('src','imgm/cons/con_'+$('.constellation_1 > .cur').data('value')+'.png');
    }
	if (last.row != 5) { now.row = last.row+1; now.col = 1; pageMove(towards.up);}	
}
...
function goPrev(){
	...
	last.col = now.col;
    if(last.row==3){
    $('.loading-line').children('.loading-main').removeClass('pt-page-mkloading');
    }
	if (last.row!=1) { now.row = last.row-1; now.col = 1; pageMove(towards.down);}	
}
/*mother end*/
